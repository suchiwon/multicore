...
#ifdef PROJECT3_PART2

typedef struct _buf_pool_init_instance_args {
    
	buf_pool_t*	buf_pool;	 
	ulint		buf_pool_size;	
	ulint		instance_no;
} buf_pool_init_instance_args;
...
UNIV_INTERN
void*
thread_func_buf_pool_init_instance(void* arg) {
    buf_pool_init_instance_args argument = *((buf_pool_init_instance_args*)arg);

    ulint checker = buf_pool_init_instance(argument.buf_pool, argument.buf_pool_size, argument.instance_no);
	...
	ulint	n_instances)	/*!< in: number of instances */
{
...
#ifdef PROJECT3_PART2    
    buf_pool_init_instance_args args[n_instances];

    pthread_t threads[n_instances];
    ...
    for (i = 0; i < n_instances; i++) {
        ...
        if (pthread_create(&threads[i], NULL, thread_func_buf_pool_init_instance, &args[i]) < 0) {
            printf("pthread_create in buf_pool_init ERROR!\n");
            return(DB_ERROR);
        }
    }
    ...
    for (i = 0; i < n_instances; i++) {
        pthread_join(threads[i], (void**)&check_init);

        if (check_init != DB_SUCCESS) {
            buf_pool_free(i);
            return(DB_ERROR);
        }
    }
